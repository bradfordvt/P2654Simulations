...
void JTAGController2::__write_vector_segment(std::uint32_t adr, byte data) {
    std::uint32_t wb_addr = 0x00003000 + adr;
    bool ret = ate_inst.write(wb_addr, (std::uint32_t)data & 0x000000FF);
    if(ret == false) {
        throw AcknowledgeError(std::string("Write Error: ") + ate_inst.get_last_response());
    }
...
byte JTAGController2::__read_vector_segment(std::uint32_t adr) {
    std::uint32_t wb_addr = 0x00003000 + adr;
    if(ate_inst.read(wb_addr)) {
        return ate_inst.get_value();
    }
...
void JTAGController2::__set_chain_length(std::uint16_t count) {
    std::uint32_t wb_addr = 0x00003000 + 0x402;
    bool ret = ate_inst.write(wb_addr, count & 0x0000FFFF);
    if(ret == false) {
        throw AcknowledgeError(std::string("Write Error: ") + ate_inst.get_last_response());
    }
...
void JTAGController2::__set_state_start(std::uint8_t start) {
    std::uint32_t wb_addr = 0x00003000 + 0x400;
    bool ret = ate_inst.write(wb_addr, start & 0x0000000F);
    if(ret == false) {
        throw AcknowledgeError(std::string("Write Error: ") + ate_inst.get_last_response());
    }
...
void JTAGController2::__set_state_end(std::uint8_t end) {
    std::uint32_t wb_addr = 0x00003000 + 0x401;
    bool ret = ate_inst.write(wb_addr, end & 0x0000000F);
    if(ret == false) {
        throw AcknowledgeError(std::string("Write Error: ") + ate_inst.get_last_response());
    }
...
void JTAGController2::__set_control_register(std::uint8_t value) {
    std::uint32_t wb_addr = 0x00003000 + 0x403;
    bool ret = ate_inst.write(wb_addr, value & 0x00000001);
    if(ret == false) {
        throw AcknowledgeError(std::string("Write Error: ") + ate_inst.get_last_response());
    }
...
std::uint8_t JTAGController2::__get_status_register() {
    std::uint32_t wb_addr = 0x00003000 + 0x404;
    if(ate_inst.read(wb_addr) & 0x1) {
        return ate_inst.get_value();
    }
    else {
...
byte_array JTAGController2::__scan_vector(byte_array& tdi_vector, int count, std::uint8_t start, std::uint8_t end) {
    /* Fill the JTAGCtrlMaster data buffer memory with tdi data */
    int data_width = 8;
    int addr_width = 10;
    int num_full_words = int(count / data_width);
...
byte_array JTAGController2::ba_scan_ir(byte_array& tdi_vector, int count) {
    std::uint8_t start = JTAGStates::SI_SHIFT_IR;
    std::uint8_t end = JTAGStates::SI_RUN_TEST_IDLE;
    return __scan_vector(tdi_vector, count, start, end);
}

byte_array JTAGController2::ba_scan_dr(byte_array& tdi_vector, int count) {
    std::uint8_t start = JTAGStates::SI_SHIFT_DR;
    std::uint8_t end = JTAGStates::SI_RUN_TEST_IDLE;
    return __scan_vector(tdi_vector, count, start, end);
}

std::string JTAGController2::scan_ir(int count, std::string tdi_string) {
    if(tdi_string.length() % 2) {
        tdi_string = std::string("0") + tdi_string;
    }
    int allocated_bytes = tdi_string.length() / 2;
...
std::string JTAGController2::scan_dr(int count, std::string tdi_string) {
    if(tdi_string.length() % 2) {
        tdi_string = std::string("0") + tdi_string;
    }
    int allocated_bytes = tdi_string.length() / 2;
...
byte JTAGController2::__hex(char ch) {
    if(ch >= 'A' && ch <= 'Z') {
        return (ch - 'A') + 10;
    }
    if(ch >= '0' && ch <= '9') {
...
const char* JTAGController2::__hex_to_char(byte data) {
    static char buffer[3];
    int i;
    for(i = 0; i < 2; i++) {
        int d = i % 2 ? data & 0xF : data >> 4;
