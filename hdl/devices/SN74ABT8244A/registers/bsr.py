...
"""
from myhdl import *
from hdl.clients.jtag.cells.InputCell import InputCell
from hdl.clients.jtag.cells.OutputCell import OutputCell
...
class bsr:
    ...
    def rtl(self):
        ...
        z4_out_cell = OutputCell(self.A2[3], sis[16], ce, se, ue, self.extest,
                                 self.tck, self.tdo, oe_neg2, self.Y2_o[3], self.Y2_e[3])
        z3_out_cell = OutputCell(self.A2[2], sis[15], ce, se, ue, self.extest,
                                 self.tck, sis[16], oe_neg2, self.Y2_o[2], self.Y2_e[2])
        z2_out_cell = OutputCell(self.A2[1], sis[14], ce, se, ue, self.extest,
                                 self.tck, sis[15], oe_neg2, self.Y2_o[1], self.Y2_e[1])
        z1_out_cell = OutputCell(self.A2[0], sis[13], ce, se, ue, self.extest,
                                 self.tck, sis[14], oe_neg2, self.Y2_o[0], self.Y2_e[0])
        y4_out_cell = OutputCell(self.A1[3], sis[12], ce, se, ue, self.extest,
                                 self.tck, sis[13], oe_neg1, self.Y1_o[3], self.Y1_e[3])
        y3_out_cell = OutputCell(self.A1[2], sis[11], ce, se, ue, self.extest,
                                 self.tck, sis[12], oe_neg1, self.Y1_o[2], self.Y1_e[2])
        y2_out_cell = OutputCell(self.A1[1], sis[10], ce, se, ue, self.extest,
                                 self.tck, sis[11], oe_neg1, self.Y1_o[1], self.Y1_e[1])
        y1_out_cell = OutputCell(self.A1[0], sis[9], ce, se, ue, self.extest,
                                 self.tck, sis[10], oe_neg1, self.Y1_o[0], self.Y1_e[0])
        b4_inp_cell = InputCell(self.A2[3], sis[8], ce, se, self.tck, sis[9])
        b3_inp_cell = InputCell(self.A2[2], sis[7], ce, se, self.tck, sis[8])
        b2_inp_cell = InputCell(self.A2[1], sis[6], ce, se, self.tck, sis[7])
        b1_inp_cell = InputCell(self.A2[0], sis[5], ce, se, self.tck, sis[6])
        a4_inp_cell = InputCell(self.A1[3], sis[4], ce, se, self.tck, sis[5])
        a3_inp_cell = InputCell(self.A1[2], sis[3], ce, se, self.tck, sis[4])
        a2_inp_cell = InputCell(self.A1[1], sis[2], ce, se, self.tck, sis[3])
        a1_inp_cell = InputCell(self.A1[0], sis[1], ce, se, self.tck, sis[2])
        c2_ctl_cell = ControlCell(self.OE_NEG2, sis[0], ce, se, ue, self.extest,
                                  self.tck, sis[1], oe2)
