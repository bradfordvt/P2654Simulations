...
@block
def InstructionRegister(path, name, si, ce, se, ue, sel, reset, clock, so, di, do, width=9, monitor=False):
    """
    ...
    if not monitor:
        ...
        @instance
        def monitor_si():
            print("\t\tInstructionRegister({:s}): si".format(path + '.' + name), si)
            while 1:
                yield si
                print("\t\tInstructionRegister({:s}): si".format(path + '.' + name), si)
        ...
        @instance
        def monitor_ce():
            print("\t\tInstructionRegister({:s}): ce".format(path + '.' + name), ce)
            while 1:
                yield ce
                print("\t\tInstructionRegister({:s}): ce".format(path + '.' + name), ce)
        ...
        @instance
        def monitor_se():
            print("\t\tInstructionRegister({:s}): se".format(path + '.' + name), se)
            while 1:
                yield se
                print("\t\tInstructionRegister({:s}): se".format(path + '.' + name), se)
        ...
        @instance
        def monitor_ue():
            print("\t\tInstructionRegister({:s}): ue".format(path + '.' + name), ue)
            while 1:
                yield ue
                print("\t\tInstructionRegister({:s}): ue".format(path + '.' + name), ue)
        ...
        @instance
        def monitor_sel():
            print("\t\tInstructionRegister({:s}): sel".format(path + '.' + name), sel)
            while 1:
                yield sel
                print("\t\tInstructionRegister({:s}): sel".format(path + '.' + name), sel)
        ...
        @instance
        def monitor_reset():
            print("\t\tInstructionRegister({:s}): reset".format(path + '.' + name), reset)
            while 1:
                yield reset
                print("\t\tInstructionRegister({:s}): reset".format(path + '.' + name), reset)
        ...
        @instance
        def monitor_clock():
            print("\t\tInstructionRegister({:s}): clock".format(path + '.' + name), clock)
            while 1:
                yield clock
                print("\t\tInstructionRegister({:s}): clock".format(path + '.' + name), clock)
        ...
        @instance
        def monitor_so():
            print("\t\tInstructionRegister({:s}): so".format(path + '.' + name), so)
            while 1:
                yield so
                print("\t\tInstructionRegister({:s}): so".format(path + '.' + name), so)
        ...
        @instance
        def monitor_isr():
            print("\t\tInstructionRegister({:s}): isr".format(path + '.' + name), isr)
            while 1:
                yield isr
                print("\t\tInstructionRegister({:s}): isr".format(path + '.' + name), isr)
        ...
        @instance
        def monitor_di():
            print("\t\tInstructionRegister({:s}): di".format(path + '.' + name), di)
            while 1:
                yield di
                print("\t\tInstructionRegister({:s}): di".format(path + '.' + name), di)
        ...
        @instance
        def monitor_do():
            print("\t\tInstructionRegister({:s}): do".format(path + '.' + name), do)
            while 1:
                yield do
                print("\t\tInstructionRegister({:s}): do".format(path + '.' + name), do)
...
@block
def InstructionRegister_tb(monitor=False):
    """
    ...
    clock = Signal(bool(0))
    t = 0
    ireg_inst = InstructionRegister('TOP', 'InstructionRegister0', si, ce, se, ue, sel, reset, clock, so, di, do, width=9,
                             monitor=monitor)
    ...
    file_data = open("InstructionRegister.csv", 'w')  # file for saving data
    ...
    return ireg_inst, clkgen, stimulus, reset_signal, print_data
...
def convert():
    ...
    ireg_inst.convert(hdl="VHDL", initial_values=True, directory=vhdl_dir, name="InstructionRegister")
    verilog_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'verilog')
    if not os.path.exists(verilog_dir):
        os.mkdir(verilog_dir, mode=0o777)
    ireg_inst.convert(hdl="Verilog", initial_values=True, directory=verilog_dir, name="InstructionRegister")
    tb = InstructionRegister_tb(monitor=False)
    tb.convert(hdl="VHDL", initial_values=True, directory=vhdl_dir, name="InstructionRegister_tb")
    tb.convert(hdl="Verilog", initial_values=True, directory=verilog_dir, name="InstructionRegister_tb")


def main():
    tb = InstructionRegister_tb(monitor=True)
